...
import albumentations as albu # Image enhancement
# from albumentations import torch as AT
plt.style.use('bmh')


# seed: reproduce results
...
def draw_convex_hull(mask, mode='convex'):
    ...
    for c in contours:
        if mode=='rect': # simple rectangular
            x, y, w, h = cv2.boundingRect(c)
            cv2.rectangle(img, (x, y), (x+w, y+h), (255, 255, 255), -1)
        if mode=='convex': # minimum convex hull
            hull = cv2.convexHull(c)
            cv2.drawContours(img, [hull], 0, (255, 255, 255),-1)
        else: # minimum rectangular region
...
def get_img(x, folder: str = "train_images_525/train_images_525"):
    """
    Returns the image based on the image name and folder
    """
...
def rle_decode(mask_rle: str = "", shape: tuple = (1400, 2100)):
    """
    Decode the Run-length Encoding mask.
...
def mask2rle(img):
    ...
    Returns run length, in String format
    """
...
def visualize(image, mask, original_image=None, original_mask=None):
    ...
    If two pairs of images and masks have passed, display both.
    """
    fontsize = 14
...
def plot_with_augmentation(image, mask, augment):
    """
    Encapsulating the 'Visualize' function.
    """
...
def post_process(probability, threshold, min_size):
    """
    This is slightly different from other kernels, because here we draw our own convex hull.
    Components with fewer pixels than "min_size" are ignored for post-processing of each predicted mask
    """
...
def get_validation_augmentation():
    """
    Add padding to make the image shape divisible by 32
    """
    test_transform = [
        albu.Resize(320, 640),
        albu.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)),
...
def get_preprocessing(preprocessing_fn):
    """Structural preprocessing transformation

    Args:
        preprocessing_fn (callbale): Normalized function
            (Each pre-trained neural network can be targeted)
    Return:
...
def f_score(pr, gt, beta=1, eps=1e-7, threshold=None, activation='sigmoid'):
    """
    Args:
        pr (torch.Tensor): A list of predicted elements(Element of prediction)
        gt (torch.Tensor):  A list of elements that are to be predicted(Elements to be predicted)
        eps (float): epsilon to avoid zero division(Elements to be predicted)
        threshold: threshold for outputs binarization(The threshold used to output binary values)
    Returns:
        float: IoU (Jaccard) score(IoU score)
    """
...
dice_score_list = []
lr_rate_list = []
valid_loss_min = np.Inf # Track changes to Validation Loss
for epoch in range(1, n_epochs+1):
    ...
    for data, target in bar:
        # If CUDA is available, move tensors to the GPU for computation
        if train_on_gpu:
            data, target = data.cuda(), target.cuda()
        # Clears gradients for all optimized variables
        optimizer.zero_grad()
        # forward propagation
        output = model(data)
        # Calculate small batch loss - batch loss
        loss = criterion(output, target)
        print("loss = " + str(loss))
        # Back propagation: Calculate the gradient of loss relative to model parameters
        loss.backward()
        # One-step optimization (parameter update)
        optimizer.step()
    ...
    # Calculate training/ validation statistics
    print('Epoch: {}  Training Loss: {:.6f}  Validation Loss: {:.6f} Dice Score: {:.6f}'.format(
        epoch, train_loss, valid_loss, dice_score))
    
    # Save the model if validation loss is reduced
...
# Use a grid search to find the optimal threshold
class_params = {}
...
gc.collect()
attempts_df = pd.DataFrame(attempts, columns=['threshold', 'size', 'dice'])
print('class_params' + str(class_params))
